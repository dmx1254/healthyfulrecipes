...
import { getArticlesAfterThree } from "@/lib/api/posts";
import Image from "next/image";
import { ArticleResponse } from "@/lib/type";
import Link from "next/link";

const BottomArticleItem = async () => {
  const news: ArticleResponse[] = await getArticlesAfterThree();
//   console.log(news);
  return (
    <div className="w-full my-12 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {news?.map((art) => (
        <Link
          href={`/article/${art?.slug}-${art?.articleID}`}
          key={art?._id}
        ...
        >
          ...
          <div className="w-full flex flex-col items-start gap-2 lg:p-2">
            <p className="max-lg:hidden w-full text-xs text-zinc-500 truncate">{art?.desc}</p>
...
export default BottomArticleItem;
