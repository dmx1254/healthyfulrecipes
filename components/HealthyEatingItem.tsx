...
import { getLastThreeArticlesByUsingCat, getLastThreePostByUsingCat } from "@/lib/api/posts";
...
const HealthyEatingItem = async ({
  ...
  const articles: ArticleResponse[] = await getLastThreeArticlesByUsingCat(slug, lim);
  //   console.log(posts);
  return (
    <div className="flex max-md:flex-col items-start gap-6">
      {articles.map((article) => (
        <Link
          key={article?._id}
          href={`/healthy-eating/${article?.slug}-${article?.articleID}`}
